...
export class TelegramService {
  ...
  async help(@Ctx() ctx: Context) {
    await ctx.reply(
      'Envie uma foto de nota fiscal para registrar um gasto ou utilize /gasto <valor> <categoria> para registro manual.',
    );
  }
  ...
  async onMessage(@Ctx() ctx: Context) {
    await ctx.reply(
      'Ainda estou aprendendo a processar textos. Tente enviar uma foto ou um comando como /gasto.',
    );
  }
}
